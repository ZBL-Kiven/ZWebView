...
import com.zj.zWeb.proctol.WebErrorType
import com.zj.zWeb.proctol.WebJavaScriptIn


@Suppress("unused")
abstract class ZWebView<T : WebJavaScriptIn> @JvmOverloads constructor(c: Context, attrs: AttributeSet? = null, def: Int = 0) : WebView(c, attrs, if (def != 0) def else android.R.attr.webViewStyle) {

    companion object {
        private var cacheFileDir: String = ""
        private val isMultiProcessSuffix by lazy {
        ...
        fun onAppAttached(c: Context, progressSuffix: String) {
            ...
            if (progressSuffix.isEmpty() || progressName.endsWith(progressSuffix)) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P && isMultiProcessSuffix) try {
        ...
        private fun getProgressName(context: Context): String? {
    ...
    private val mWebViewClient = object : WebViewClient() {
        ...
        override fun shouldOverrideUrlLoading(view: WebView?, request: WebResourceRequest?): Boolean {
            return if (this@ZWebView.shouldOverrideUrlLoading(view, request)) true else super.shouldOverrideUrlLoading(view, request)
        }

        override fun onLoadResource(view: WebView?, url: String?) {
            super.onLoadResource(view, url)
            this@ZWebView.onLoadResource(view, url)
        }

        override fun onPageStarted(view: WebView?, url: String?, favicon: Bitmap?) {
            super.onPageStarted(view, url, favicon)
            this@ZWebView.onPageStarted(view, url, favicon)
        }
        ...
        override fun onPageFinished(view: WebView, url: String) {
            ...
            measure(w, h)
            this@ZWebView.onPageFinished(view, url)
        }

        override fun onReceivedHttpError(view: WebView?, request: WebResourceRequest?, errorResponse: WebResourceResponse?) {
            this@ZWebView.onReceivedHttpError(view, request, errorResponse)
        }

        override fun onReceivedSslError(view: WebView?, handler: SslErrorHandler?, error: SslError?) {
            this@ZWebView.onReceivedSslError(view, handler, error)
        }

        override fun onReceivedError(view: WebView?, request: WebResourceRequest?, error: WebResourceError?) {
            this@ZWebView.onReceivedError(view, request, error)
        }

        override fun shouldInterceptRequest(view: WebView?, request: WebResourceRequest?): WebResourceResponse? {
            return this@ZWebView.shouldInterceptRequest(view, request) ?: super.shouldInterceptRequest(view, request)
        }
    }
    ...
    private val mWebChromeClient = object : WebChromeClient() {
        ...
        override fun onReceivedTitle(view: WebView?, title: String?) {
            ...
            this@ZWebView.onReceivedTitle(view, title)
        }

        override fun onProgressChanged(view: WebView?, newProgress: Int) {
            super.onProgressChanged(view, newProgress)
            this@ZWebView.onProgressChanged(view, newProgress)
        }

        override fun getDefaultVideoPoster(): Bitmap? {
            return this@ZWebView.getDefaultVideoPoster() ?: super.getDefaultVideoPoster()
        }
    }
    ...
    open fun onError(type: WebErrorType) {
        ...
        onLog("${type.name} : desc = $s")
    }

    open fun onLog(s: String) {
        Log.e("ZWebView", s)
    }
